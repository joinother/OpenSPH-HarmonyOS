...
namespace lab {
namespace {
double hash(int x,int y,int z){uint32_t h=uint32_t(x)*374761393u+uint32_t(y)*668265263u+uint32_t(z)*2147483647u;h=(h^(h>>13))*1274126177u;return double(h^(h>>16))/4294967295.;}
...
PlanetTexel planetTexel(int style,double x,double y,double z){
 const double terrain=fbm(x*2.7+9,y*2.7+3,z*2.7-4,6),detail=fbm(x*28,y*28,z*28,3);
 // Warp longitude with latitude-dependent winds, while keeping the field spherical.
 const double warp=.24*std::sin(y*7)+.35*fbm(x*3+30,y*3,z*3,3);
 const double wx=x*std::cos(warp)-z*std::sin(warp),wz=x*std::sin(warp)+z*std::cos(warp);
 double cloud=smooth(.49,.69,fbm(wx*9+41,y*10+15,wz*9,5));
 PlanetTexel p{{0,0,0},0,float(cloud)};
 if(style==0){double cells=fbm(x*55,y*55,z*55,3);p.color=color(.90+.10*cells,.35+.46*cells,.045+.24*cells);p.cloud=0;return p;}
 if(style==1){
 ...
 for(int i=0;i<28;++i){double cy=hash(i,9,2)*2-1,a=hash(i,2,8)*6.28318530718,r=std::sqrt(1-cy*cy);double d=std::sqrt(std::max(0.,2-2*(x*r*std::cos(a)+y*cy+z*r*std::sin(a))));double radius=.018+.12*hash(i,7,4);double bowl=1-smooth(radius*.65,radius,d),rim=std::exp(-std::pow((d-radius)/(radius*.1),2));for(auto &c:p.color)c=float(c*(1-.35*bowl)+.13*rim);}

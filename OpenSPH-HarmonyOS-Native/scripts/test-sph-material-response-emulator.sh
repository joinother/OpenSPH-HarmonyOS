...
"$sdk/native/llvm/bin/clang++" --target=aarch64-linux-ohos --sysroot="$sdk/native/sysroot" \
 ...
 "$project_dir/tests/sph_material_response_test.cpp" "$project_dir/entry/src/main/cpp/engine.cpp" "$project_dir/entry/src/main/cpp/galaxy_simulation.cpp" "$project_dir/entry/src/main/cpp/orbit.cpp" "$core_build/core/libcore.a" \
 -pthread -static-libstdc++ -o "$core_build/sph_material_response_test"
hdc="$sdk/toolchains/hdc"
"$hdc" -t "$device" file send "$core_build/sph_material_response_test" /data/local/tmp/sph-material-response-test
"$hdc" -t "$device" shell chmod 755 /data/local/tmp/sph-material-response-test
"$hdc" -t "$device" shell mkdir -p /data/local/tmp/sph-material-response-results
result="$("$hdc" -t "$device" shell /data/local/tmp/sph-material-response-test /data/local/tmp/sph-material-response-results)"
printf '%s\n' "$result"
[[ "$result" == *"PASS SPH material response"* && "$result" != *"FAIL "* ]]

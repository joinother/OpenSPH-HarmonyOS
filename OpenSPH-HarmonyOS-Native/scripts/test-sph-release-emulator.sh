...
"$sdk/native/llvm/bin/clang++" --target=aarch64-linux-ohos --sysroot="$sdk/native/sysroot" \
  -std=c++17 -O3 -DNDEBUG -I"$project_dir/third_party/opensph/core" -I"$project_dir/entry/src/main/cpp" "$project_dir/tests/sph_release.cpp" \
  "$build_dir/core/libcore.a" -pthread -static-libstdc++ -o "$build_dir/sph_release"
hdc="$sdk/toolchains/hdc"
"$hdc" -t "$target" file send "$build_dir/sph_release" /data/local/tmp/opensph_sph_release >&2
"$hdc" -t "$target" shell chmod 755 /data/local/tmp/opensph_sph_release >&2
...
"$hdc" -t "$target" shell /data/local/tmp/opensph_sph_release > "$build_dir/sph-release.jsonl"
node -e 'const fs=require("fs"),s=fs.readFileSync(process.argv[1],"utf8");if(s.includes("FAIL")||!JSON.parse(s.trim().split("\n").at(-1)).ok)process.exit(1)' "$build_dir/sph-release.jsonl"
cat "$build_dir/sph-release.jsonl"

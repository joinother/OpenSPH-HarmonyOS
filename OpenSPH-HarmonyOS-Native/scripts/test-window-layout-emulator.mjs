...
async function inspect(name,folded,orientation,reference){
 ...
 const viewport=get('universe-viewport'),dock=get('workspace-transport'),header=get('workspace-header'),drawer=s.ui.panelOpen?get('workspace-drawer'):[0,0,0,0];
 ...
 const remote='/data/local/tmp/sph-window-'+name+'.png',file=project+'../OpenSPH-0.14.0-'+name+'.png';h('shell','uitest','screenCap','-p',remote);h('file','recv',remote,file);h('shell','rm',remote);
 ...
 writeFileSync(project+'docs/evidence/fold-rotation-'+name+'-state.json',JSON.stringify(s,null,2)+'\n');return result;
}
const results=[];
...
try{
 ...
 console.log(JSON.stringify({ok:true,device,checks:['actual fold/expand and main-window rotation','native viewport matches safe area','actual UI bounds and panel/dock separation','near body visible above UI','editor and placement drafts preserved','scene revision, replay frame and camera preserved'],results},null,2));
}finally{

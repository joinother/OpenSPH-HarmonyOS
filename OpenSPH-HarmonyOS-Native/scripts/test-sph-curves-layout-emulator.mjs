...
function capture(name){const file=root+'../OpenSPH-'+version+'-sph-curve-'+name+'.png';rmSync(file,{force:true});h('shell','rm','-f','/data/local/tmp/sph-diagnostic-layout.png');h('shell','uitest','screenCap','-p','/data/local/tmp/sph-diagnostic-layout.png');h('file','recv','/data/local/tmp/sph-diagnostic-layout.png',file);}
const results=[];
try{
 ...
 for(const[name,fold,orientation]of [['wide',false,'portrait'],['phone',true,'portrait'],['landscape',true,'landscape']]){
  ...
  for(const metric of ['pressure','internal','damage','kinetic','energy']){
 ...
 console.log(JSON.stringify({ok:true,device,checks:['five curve metrics and both extrema actually tapped in expanded, folded portrait and folded landscape','visible color legends and units','guarded extrema seek; definition camera pose and native surface preserved','immersive window and no rendering errors'],results},null,2));

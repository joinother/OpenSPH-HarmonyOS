...
function capture(name){const file=root+'../OpenSPH-'+version+'-sph-release-'+name+'.png';rmSync(file,{force:true});h('shell','rm','-f','/data/local/tmp/sph-diagnostic-layout.png');h('shell','uitest','screenCap','-p','/data/local/tmp/sph-diagnostic-layout.png');h('file','recv','/data/local/tmp/sph-diagnostic-layout.png',file);}
const results=[];
try{
 call('listCommands');call('getUiState');
 call('uiAction',{action:'theme.sphere-release'},['--wait-state','paused']);call('start',{},['--wait-state','completed']);action('ui.restore');
 for(const[name,fold,orientation]of [['wide',false,'portrait'],['phone',true,'portrait'],['landscape',true,'landscape']]){
  ...
  action('panel.close');action('panel.parameters');action('color.0');await delay(1200);
  ...
  call('setUiValue',{field:'scene.speed',value:2});call('setUiValue',{field:'body.spin',value:.004});
  const button=visible('sph-stationary');tap(button);const after=call('getState');assert.equal(after.definition.config.speed,0);assert.equal(after.definition.config.targetSpin,0);assert.equal(after.simulation.config.speed,0);assert.equal(after.simulation.time,before.simulation.time);assert.deepEqual(after.camera,before.camera);assert.equal(after.rendering.sceneRevision,before.rendering.sceneRevision);taps.push({action:'scene.stationary',bounds:bounds(button)});
  action('panel.close');action('panel.observe');await delay(1000);visible('sph-window-summary');visible('径向速度绝对峰值');
  const summary=call('getSphWindowSummary').summary;assert.ok(summary.available&&summary.includesInitial);const viewed=call('getState');assert.equal(viewed.window.immersive,true);assert.equal(viewed.rendering.error,'');capture(name);
 ...
 console.log(JSON.stringify({ok:true,device,checks:['stationary shortcut physically tapped in expanded, folded portrait and landscape','both spin draft fields cleared while active history and camera remain unchanged','window statistics visible in all three layouts','immersive window without render errors'],results},null,2));

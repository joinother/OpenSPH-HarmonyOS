...
function capture(name){const file=root+'../OpenSPH-'+version+'-sph-structure-'+name+'.png';rmSync(file,{force:true});h('shell','rm','-f','/data/local/tmp/sph-diagnostic-layout.png');h('shell','uitest','screenCap','-p','/data/local/tmp/sph-diagnostic-layout.png');h('file','recv','/data/local/tmp/sph-diagnostic-layout.png',file);}
const results=[];
try{
 call('listCommands');call('getUiState');
 call('setScene',{preset:0,count:600,speed:5,angle:0,duration:20,selfGravity:true,targetRadiusKm:100,impactorRadiusKm:60,targetDensity:2700,impactorDensity:2700,targetSpin:0,seed:1234},['--wait-state','paused']);call('start',{},['--wait-state','completed']);action('ui.restore');
 for(const[name,fold,orientation]of [['wide',false,'portrait'],['phone',true,'portrait'],['landscape',true,'landscape']]){
  ...
  for(const metric of ['gravity','relativeKinetic','radius','radial']){
    const button=visible('sph-curve-'+metric);tap(button);const after=call('getSphObservation');assert.equal(after.metric,metric);assert.ok(after.plot.available);taps.push({metric,bounds:bounds(button)});
  }
  visible('sph-curve-plot');capture(name);
  for(const extreme of ['minimum','maximum']){
    const curve=call('getSphObservation'),button=visible('sph-curve-'+extreme);tap(button);const after=call('getState');assert.equal(after.simulation.time,extreme==='minimum'?curve.plot.minTime:curve.plot.maxTime);
    assert.deepEqual(after.camera,before.camera);assert.deepEqual(after.definition,before.definition);assert.equal(after.rendering.sceneRevision,before.rendering.sceneRevision);assert.equal(after.rendering.surfaceStarts,before.rendering.surfaceStarts);assert.equal(after.window.immersive,true);assert.equal(after.rendering.error,'');taps.push({extreme,time:after.simulation.time,bounds:bounds(button)});
  }
 ...
 console.log(JSON.stringify({ok:true,device,checks:['four structure metrics and both extrema actually tapped in expanded, folded portrait and folded landscape','signed radial curve and physical units','guarded extrema seek; definition camera pose and native surface preserved','immersive window and no rendering errors'],results},null,2));

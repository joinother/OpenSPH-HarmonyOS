...
"$sdk/native/llvm/bin/clang++" --target=aarch64-linux-ohos --sysroot="$sdk/native/sysroot" \
  -std=c++14 -O3 -DNDEBUG -I"$project_dir/third_party/opensph/core" "$project_dir/tests/sph_baseline.cpp" \
  "$build_dir/core/libcore.a" -pthread -static-libstdc++ -o "$build_dir/sph_baseline"
hdc="$sdk/toolchains/hdc"
"$hdc" -t "$target" file send "$build_dir/sph_baseline" /data/local/tmp/opensph_sph_baseline >&2
"$hdc" -t "$target" shell chmod 755 /data/local/tmp/opensph_sph_baseline >&2
...
"$hdc" -t "$target" shell /data/local/tmp/opensph_sph_baseline > "$build_dir/sph-baseline.jsonl"
node "$project_dir/scripts/analyze-sph-baseline.mjs" "$build_dir/sph-baseline.jsonl" >&2
cat "$build_dir/sph-baseline.jsonl"

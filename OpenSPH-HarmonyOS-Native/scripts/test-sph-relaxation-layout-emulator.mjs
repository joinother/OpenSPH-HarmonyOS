...
function capture(name){const file=root+'../OpenSPH-'+version+'-sph-relaxation-'+name+'.png';rmSync(file,{force:true});h('shell','rm','-f','/data/local/tmp/sph-diagnostic-layout.png');h('shell','uitest','screenCap','-p','/data/local/tmp/sph-diagnostic-layout.png');h('file','recv','/data/local/tmp/sph-diagnostic-layout.png',file);}
const results=[];
try{
 call('listCommands');call('getUiState');
 call('uiAction',{action:'theme.direct-impact'},['--wait-state','paused']);assert.equal(call('getState').simulation.config.relaxationSeconds,undefined);call('uiAction',{action:'theme.compare'},['--wait-state','paused']);assert.equal(call('getState').simulation.config.relaxationSeconds,16);action('ui.restore');
 for(const[name,fold,orientation]of [['wide',false,'portrait'],['phone',true,'portrait'],['landscape',true,'landscape']]){
  ...
  for(const seconds of [0,16,64]){
    const button=visible('sph-relax-'+seconds);tap(button);const after=call('getState');assert.equal(after.definition.config.relaxationSeconds??0,seconds);assert.equal(after.simulation.config.relaxationSeconds,16);
    assert.deepEqual(after.camera,before.camera);assert.equal(after.simulation.time,0);assert.equal(after.rendering.sceneRevision,before.rendering.sceneRevision);assert.equal(after.window.immersive,true);assert.equal(after.rendering.error,'');taps.push({seconds,bounds:bounds(button)});
  }
  capture(name);
 ...
 console.log(JSON.stringify({ok:true,device,checks:['library direct/prepared pair applies real model parameters','three preparation options actually tapped in expanded, folded portrait and folded landscape','draft options leave collision frame/time, native revision and camera intact','immersive window and no rendering errors'],results},null,2));

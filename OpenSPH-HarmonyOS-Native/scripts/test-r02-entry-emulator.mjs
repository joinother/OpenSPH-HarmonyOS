...
try{
 ...
 const catalog=await call('getUiState');for(const [id,label] of [['theme.material-fractured','岩体撞击：预先受损'],['theme.galaxy-retrograde','星系相遇：次星系反向旋转'],['project.save','保存设置与视角']])assert.equal(catalog.actions.find(a=>a.id===id).label,label);check('Theme and save-setting labels exposed through the same UI catalog');

...
try{
  action('preset.5','paused');let catalog=call('getUiState');assert.ok(catalog.actions.length>45);assert.equal(catalog.fields.length,33);assert.ok(catalog.fields.some(f=>f.field==='trace.impulse'));assert.ok(catalog.fields.some(f=>f.field==='material.exposure'));

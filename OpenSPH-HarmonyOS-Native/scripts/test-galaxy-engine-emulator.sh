...
"$hdc" -t "$device" shell chmod 755 /data/local/tmp/sph-galaxy-engine-test
"$hdc" -t "$device" shell mkdir -p /data/local/tmp/sph-galaxy-engine-results
result="$("$hdc" -t "$device" shell /data/local/tmp/sph-galaxy-engine-test /data/local/tmp/sph-galaxy-engine-results)"
printf '%s\n' "$result"
[[ "$result" == *"PASS galaxy engine"* && "$result" != *"FAIL "* ]]

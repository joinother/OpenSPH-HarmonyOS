...
try{
 ...
 h('shell','snapshot_display','-f','/data/local/tmp/sph-impact-local.jpeg');h('file','recv','/data/local/tmp/sph-impact-local.jpeg','docs/evidence/impact-tides-0.55.0/local-tides.jpeg');
 ...
 console.log(JSON.stringify({ok:true,device,checks:['real incoming state to SPH particles','initial mass and kinetic budget','60-second material evolution','lossy and duplicate actions rejected','v19 external field and diagnostics','exact parent state/history/camera/surfaces restored','loaded replay can exit to new experiment'],source:source.contact,initial:initial.simulation,final:done.simulation,returnChecks:{time:true,frames:true,vectors:true,camera:true,appearance:true,surfaces:true}},null,2));

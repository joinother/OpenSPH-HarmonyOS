...
> 类型：当前参考；适用版本：0.29.0；实现／验证日期：2026-09-07（Asia/Shanghai）。
...
2. 0.29.0 已接入五项曲线、采样极值定位与全部十项 CSV 导出；跨实验叠加及真实物理事件识别仍待做。下一步用标准冲击和弹性算例建立分辨率／时间步收敛证据。先确认能量预算中缺少的弹性项，不能简单用动能加内能作误差百分比。

...
test('main viewport placement uses the presented plane, aim velocity and one cancellable transaction',async()=>{
 ...
 assert.equal(app.placementFields[1],'2.250000');assert.equal(app.placementFields[2],'60.000000');assert.deepEqual(calls.filter(c=>c[0]==='viewportPlacement').at(-1),['viewportPlacement',.62,.36,0]);

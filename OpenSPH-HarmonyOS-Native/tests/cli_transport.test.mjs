...
test('theme catalog isolates conditions and collision speed comparison changes only speed',async()=>{
  const {page:app}=page();
  const catalog=JSON.parse(await app.execute('listExperiments',{}));assert.equal(catalog.catalogVersion,1);assert.equal(catalog.experiments.length,23);
  ...
  for(const t of catalog.experiments){await app.execute('uiAction',{action:'theme.'+t.id});assert.equal(app.themeState().id,t.id);assert.equal(app.themeState().modified,false);assert.equal(app.panelOpen,false);assert.equal(app.colorMode,t.id.startsWith('material-')?7:0);}
});
...
test('material state changes solver inputs, shares UI fields, and comparison restores controlled conditions',async()=>{
 const {page:app,calls,state}=page();
 await app.execute('uiAction',{action:'theme.material-hot'});assert.equal(app.config().initialEnergyMJkg,1.7);assert.equal(app.colorMode,7);assert.equal(state.lastStartedConfig.initialEnergyMJkg,1.7);
 const hot=JSON.parse(JSON.stringify(app.config()));await app.execute('uiAction',{action:'theme.compare'});assert.equal(app.config().initialEnergyMJkg,undefined);assert.equal(app.themeState().id,'material-cold');assert.equal(app.config().speed,hot.speed);
 const starts=calls.filter(c=>c[0]==='start').length;
 await app.execute('setUiValue',{field:'scene.energy',value:3.4});await app.execute('setUiValue',{field:'scene.damage',value:.9});assert.equal(app.dirty,true);assert.equal(calls.filter(c=>c[0]==='start').length,starts);assert.equal(app.themeState().modified,true);
 await app.execute('uiAction',{action:'simulation.apply'});assert.equal(state.lastStartedConfig.initialDamage,.9);
 const before=JSON.stringify(app.config());for(const extra of [{initialEnergyMJkg:null},{initialDamage:null},{initialEnergyMJkg:-1},{initialEnergyMJkg:6.81},{initialDamage:1.1},{initialEnergyMJkg:'1'},{selfGravity:true,relaxationSeconds:16}]){await assert.rejects(app.execute('setScene',{...app.config(),...extra}));assert.equal(JSON.stringify(app.config()),before);}
 await app.execute('uiAction',{action:'scene.gravity'});await assert.rejects(app.execute('uiAction',{action:'scene.relax.16'}));
 app.choose(4);assert.equal(app.config().initialEnergyMJkg,undefined);assert.equal(app.config().initialDamage,undefined);await assert.rejects(app.execute('setUiValue',{field:'scene.energy',value:1}));
});

test('thermal views retain physics and camera, gate historical data, and preserve material recipe',async()=>{
 const {page:app,state,calls}=page();await app.execute('uiAction',{action:'theme.material-fractured'});
 state.sph={available:true,response:{model:'basalt-von-mises-response-v1',meltEnergyMJkg:3.4,intactYieldGPa:3.5,softeningMean:.5,zeroShearMassFraction:.2,damagedMassFraction:.3,strengthMean:.25}};
 const starts=calls.filter(c=>c[0]==='start').length;
 for(const color of [7,8]){await app.execute('uiAction',{action:'color.'+color});assert.equal(app.colorMode,color);assert.equal(calls.filter(c=>c[0]==='start').length,starts);}

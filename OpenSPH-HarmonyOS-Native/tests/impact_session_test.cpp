...
int main(int argc,char** argv){try{
 ...
 require(e.saveReplay(argv[1]),"v16 save failed");std::ifstream f(path,std::ios::binary);std::string bytes((std::istreambuf_iterator<char>(f)),{});require(u32(bytes,4)==16,"missing v16");
 std::vector<size_t> incoming,extensions;size_t p=88;const auto n=u32(bytes,8);
 for(uint32_t k=0;k<n;k++){p+=84;extensions.push_back(p);if(u32(bytes,p)==1)incoming.push_back(p+4);p+=u32(bytes,p)?132:4;auto count=u32(bytes,p);p+=4;for(uint32_t i=0;i<count;i++){auto len=u32(bytes,p);p+=4+len+72;}auto trails=u32(bytes,p);p+=4+trails*sizeof(Particle);}
 require(p==bytes.size()&&!incoming.empty(),"format mismatch");
 require(e.loadReplay(argv[1]),"v16 load failed");same(hit.contact,e.status().contact);e.seek(0);require(!makeImpactPlan(e.status().contact).available,"initial frame invented event");e.seek(n-1);same(hit.contact,e.status().contact);
 const size_t offset=incoming.back();for(int variant=0;variant<6;variant++){auto bad=bytes;double value=0;
 ...
 write(bytes);require(e.loadReplay(argv[1]),"restore v16 failed");e.orbitClock(.001);e.pause(false);wait(e,[&](Status s){return s.time>hit.time;});e.pause(true);same(hit.contact,e.status().contact);
 auto legacy=bytes;for(auto it=extensions.rbegin();it!=extensions.rend();++it)legacy.erase(*it,u32(bytes,*it)?132:4);uint32_t v14=14;std::memcpy(&legacy[4],&v14,4);write(legacy);
 ...
 std::cout<<"PASS actual 3-body orbit incoming capture at "<<plan.timeSeconds<<" seconds, "<<plan.relativeSpeedKmS<<" km/s; v16 exact roundtrip, seek, six corrupt transactional rejections, resume immutability, v14 contact compatibility\n";
 e.cancel();

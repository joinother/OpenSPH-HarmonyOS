...
int main(int argc,char** argv){try{
 ...
 require(e.saveReplay(argv[1],true,false),"v17 save failed");std::string path=std::string(argv[1])+"/last-replay.osphr";std::ifstream f(path,std::ios::binary);std::string bytes((std::istreambuf_iterator<char>(f)),{});uint32_t version;std::memcpy(&version,bytes.data()+4,4);require(version==17,"missing source replay version");

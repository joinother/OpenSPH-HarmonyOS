...
int main(){
 ...
 for(int color=3;color<=6;++color){Camera diagnostic=b;diagnostic.color=color;n.request(2,diagnostic,false,0,false,true);tick(n,2,0);assert(n.displayed().color==color);}
 Camera invalidColor=b;invalidColor.color=7;bool colorRejected=false;try{n.request(2,invalidColor,false,0,false,true);}catch(...){colorRejected=true;}assert(colorRejected);

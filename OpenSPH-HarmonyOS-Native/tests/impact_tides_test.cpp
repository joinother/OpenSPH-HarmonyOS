...
int main(int argc,char** argv){try{
 ...
 check(e.saveReplay(argv[1],true,false),"v19 save");const std::string path=std::string(argv[1])+"/last-replay.osphr";std::ifstream input(path,std::ios::binary);std::string bytes((std::istreambuf_iterator<char>(input)),{});uint32_t version;std::memcpy(&version,bytes.data()+4,4);check(version==19,"v19 missing");

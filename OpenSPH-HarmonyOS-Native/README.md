...
- 自定义系统可启用球体接触：独立编辑质量和半径，按表面距离连续检测、弹性反弹并自动暂停，近距增加回放采样；“球体接触实验”直接提供两个带外观球体的接近演示，新版 v14 保存完整轨道状态，兼容旧 v11 半径与最后接触记录。尚无流体破裂、合并或残骸返回，见 [接触模型](docs/reference/SPHERE-CONTACT.md)。

...
> 类型：当前项目概览；源码版本：0.49.0；更新日期：2026-09-08（Asia/Shanghai）。
...
- 自定义系统可直接在主星图轻点或拖动放置天体，双指缩放；候选带外观与相对速度箭头，可设置瞄准已有天体、取消或确认。确认仍重建初始条件；自定义系统可启用实体半径与理想弹性接触，见 [放置与碰撞接入](docs/reference/VIEWPORT-PLACEMENT.md)。

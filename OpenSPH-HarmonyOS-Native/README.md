...
> 类型：当前项目概览；源码版本：0.46.0；更新日期：2026-09-08（Asia/Shanghai）。
...
- 主题实验库提供十八个主题（复用原创概念封面、原生模拟投影封面与 NASA 月面地图封面）：慢撞、快撞、错位相撞、旋转岩石、蔚蓝世界、三种行星探索、“环影之间”、“环为什么会错位”和“给环一点额外速度”和“月海与高地”，以及“碰撞放大镜”的速览／细看两组和直接／预松弛碰撞两组，以及静止岩球自由演化两组；包含观察目标、速度／粒子预算对照、曲线入口和恢复初始条件。
...
星系实验使用两个扩展解析势与 200–2400 个无质量恒星示踪点，独立采用 kpc／Myr；点数不改变引力质量，未使用实体球碰撞、气体或自洽树形引力。每 4 Myr 记录一帧，50–800 Myr 共最多 201 帧，v12 是观察快照。完整定义见 [星系模型](docs/reference/GALACTIC-TIDES.md)。
...
0.46.0 盘内天空、地表观测、固定身份回放及三布局验收见 [本轮记录](docs/releases/RELEASE-0.46.0.md)。星系参照、共享曲线与双实验 CSV 的历史验收见 [0.45.0 记录](docs/releases/RELEASE-0.45.0.md)。底层潮汐力学定义沿用 0.43.0；相遇预览验收保留在 [0.44.0 记录](docs/releases/RELEASE-0.44.0.md)。

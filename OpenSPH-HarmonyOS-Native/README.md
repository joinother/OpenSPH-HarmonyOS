...
> 类型：当前项目概览；源码版本：0.45.0；更新日期：2026-09-08（Asia/Shanghai）。
...
- 新增“两条潮汐尾／反转一片星系”：可调质量比、偏移、盘倾角、接近速度与旋转方向；恒星群按扩展势引力积分，支持中心跟随、诊断、命名实验和 v12 回放。可在星图预览并拖动调整初始错身距离，取消保留原历史；五条演化曲线支持采样极值定位，CLI 导出带已应用参数的原始 CSV。0.45.0 可保存本地星系参照，跨实验／重启叠加五条曲线，查看参数差异及同一时刻差值，并导出两组记录。见 [星系潮汐](docs/reference/GALACTIC-TIDES.md)。这是限制性形态模型，尚无气体、自洽暗晕或完整合并。
...
0.45.0 的星系参照、共享曲线、双实验 CSV 和三布局验收见 [本轮记录](docs/releases/RELEASE-0.45.0.md)。底层潮汐力学定义沿用 0.43.0；相遇预览验收保留在 [0.44.0 记录](docs/releases/RELEASE-0.44.0.md)。

...
> 类型：当前项目概览；源码版本：0.26.0；更新日期：2026-09-07（Asia/Shanghai）。
...
后续批次细化见 [宇宙沙盒博客与论坛专题](docs/research/UNIVERSE-SANDBOX-BLOG-FORUM-2026-09-07.md)：实验配方保存已在 0.21.0 建立基础，0.25.0 已加入单行星质量／外观复制放置，0.26.0 已加入单次实验的距离／速率曲线，继续跨实验结果对照和整组配方复用，再做卫星扰动和 SPH 诊断，逐步进入自引力碰撞。文中 32 项建议与 14 个实验方案均标明现有基础、依赖及验收门槛，尚未实现的部分不计入当前能力。

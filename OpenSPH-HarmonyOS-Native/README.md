...
- 自定义系统可启用球体接触：独立编辑质量和半径，按表面距离连续检测、弹性反弹并自动暂停，近距增加回放采样；“球体接触实验”直接提供两个带外观球体的接近演示，v11 保存半径与最后接触记录。尚无流体破裂、合并或残骸返回，见 [接触模型](docs/reference/SPHERE-CONTACT.md)。
...
岩质 SPH 实验使用默认玄武岩材料路径，关闭材料自引力时粒子预算为 200–2400；开启时为 200–1200，使用与 SPH 核一致的软化引力逐对求和。初始球体尚未建立静力平衡。行星轨道使用双精度牛顿点质量引力与 leapfrog，时长 1–10 儒略年；未指定半径的点质量模式保留近距停止；显式半径模式使用无摩擦、恢复系数 1 的理想弹性球体响应，尚无行星碰撞合并、破碎或 SPH 与轨道模型联动。

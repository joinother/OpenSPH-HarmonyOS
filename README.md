...
本分支保存星体实验室 0.56.0 的 ArkTS/ArkUI 原生检查点，继续开发请进入 [OpenSPH-HarmonyOS-Native](OpenSPH-HarmonyOS-Native)，恢复步骤和本轮验证见 [检查点说明](CHECKPOINT.md)。以下正文保留旧 wxWidgets 桌面移植路线；两个工程分别维护。
...
当前可查看每个 SPH 团块来自双方的实际质量比例，支持分页 CSV 与 v20 来源回放；隔离／冻结潮汐局部撞击仍可返回原世界。见 [检查点说明](CHECKPOINT.md)。

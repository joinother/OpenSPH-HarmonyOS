...
记录日期：2026-09-07（Asia/Shanghai）；源码版本：0.29.0。
...
- 0.29.0 新增 SPH 五指标曲线、采样极值定位和原始 CSV／来源导出。85 项主机测试、原生求解与回放回归、三类真实 SPH 曲线、三种窗口实际点击、旧 UI／CLI 完整回归通过；已恢复本轮开始时的一年环形行星轨道与视角，保留十个命名实验并清理临时备份。上一版未完成的窗口验收已补做。见 [0.29.0 记录](OpenSPH-HarmonyOS-Native/docs/releases/RELEASE-0.29.0.md)。

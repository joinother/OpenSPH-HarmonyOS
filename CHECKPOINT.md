...
记录日期：2026-09-07（Asia/Shanghai）；源码版本：0.28.0。
...
- 0.28.0 接入真实 SPH 压力／比内能／有效材料损伤、双精度质量加权读数、粒子着色、CLI 与 v5 回放；81 项主机测试、原生求解／回放回归和三类桌面同初值对照通过。应用已构建安装；最终横屏复验、完整旧 UI／CLI 回归和现场恢复因模拟器交错操作待继续，不声称最终验收完成。见 [0.28.0 检查点记录](OpenSPH-HarmonyOS-Native/docs/releases/RELEASE-0.28.0.md)。
